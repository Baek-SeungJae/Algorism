...
public class O20200712 {
	public static void main(String[] args) {
		...
		System.out.println(
				"Enter a command: i(insert), r(emove), s(earch), inorder, p(rint), h(eight), nc(node count), w(idth), k(th smallest node), c(count trees),or q(uit)");
...
class BSTree<T extends Comparable<T>> {
	...
	private void widthCount(BSTNode<T> tree, int i, ArrayList<Integer> widthlist) {
		if (widthlist.size() <= i)
			...
			widthlist.set(i, widthlist.get(i) + 1);
		if (tree.left != null)
			widthCount(tree.left, i + 1, widthlist);
		if (tree.right != null)
			widthCount(tree.right, i + 1, widthlist);
	}
	...
	public BSTNode<T> kthSmallestNode(int kth) {
		...
		if (isEmpty())
			...
			return inorder4kth(root, kth, count);
	}
